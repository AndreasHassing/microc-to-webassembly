...
let rec addCST i C =
    ...
    // >=
    | (x, CSTI y :: LT :: NOT :: C1) ->
        if x >= y then addCST 1 C1 else addCST 0 C1
    // <
    | (x, CSTI y :: LT :: C1) ->
        if x <  y then addCST 1 C1 else addCST 0 C1
    // <=
    | (x, CSTI y :: SWAP :: LT :: NOT :: C1) ->
        if x <= y then addCST 1 C1 else addCST 0 C1
    // >
    | (x, CSTI y :: SWAP :: LT :: C1) ->
        if x >  y then addCST 1 C1 else addCST 0 C1
...
let addIFZERO lab3 C =
    match C with
    | GOTO lab1 :: Label lab2 :: C1 when lab2 = lab3 -> IFNZRO lab1 :: Label lab3 :: C1
    | _ -> IFZERO lab3 :: C

let addIFNZRO lab3 C =
    match C with
    | GOTO lab1 :: Label lab2 :: C1 when lab2 = lab3 -> IFZERO lab1 :: Label lab3 :: C1
    | _ -> IFNZRO lab3 :: C
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) (C : instr list) : instr list =
    ...
    | Andalso(e1, e2) ->
      ...
      | IFNZRO labthen :: C1 ->
        ...
        cExpr e1 varEnv funEnv
           (addIFZERO labelse (cExpr e2 varEnv funEnv (addIFNZRO labthen C2)))
      | _ ->
        let (jumpend,  C1) = makeJump C
    ...
    | Orelse(e1, e2) ->
      match C with
      | IFNZRO lab :: _ ->
        cExpr e1 varEnv funEnv (addIFNZRO lab (cExpr e2 varEnv funEnv C))
      | IFZERO labthen :: C1 ->
        let(labelse, C2) = addLabel C1
        cExpr e1 varEnv funEnv
           (addIFNZRO labelse (cExpr e2 varEnv funEnv (addIFZERO labthen C2)))
      | _ ->
        let (jumpend, C1) = makeJump C
        let (labtrue, C2) = addLabel(addCST 1 C1)
        cExpr e1 varEnv funEnv
           (addIFNZRO labtrue (cExpr e2 varEnv funEnv (addJump jumpend C2)))
